predconfPois_homog <- function(region, object, level, nCores,
                         what=c("estimate", "se",
                                "confidence", "prediction"),
                         new.coef=NULL) {
  what <- match.arg(what)
  stopifnot(0 < level && level < 1)
  lam <- predict(object, window=region, new.coef=new.coef) * nCores
  ...
  sd.muhat <- sqrt(var.muhat) / nCores #sqrt(nCores)
