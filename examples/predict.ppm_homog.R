predict.ppm_homog <- function (object, nCores, window = NULL, ngrid = NULL, locations = NULL, 
          ...
          repair = TRUE) 
{
  ...
  if (type == "count") {
    if (is.null(window)) {
      if (!seonly) 
        est <- predconfPois_homog(NULL, model, level, estimatename, nCores = nCores,
                            new.coef = new.coef)
      if (se) 
        sem <- predconfPois_homog(NULL, model, level, "se", nCores = nCores,
                            new.coef = new.coef)
    }
    else if (is.tess(window)) {
      tilz <- tiles(window)
      if (!seonly) {
        est <- lapply(tilz, predConfPois_homog, object = model, nCores = nCores,
      ...
      if (se) 
        sem <- sapply(tilz, predConfPois_homog, object = model, nCores = nCores,
                      level = level, what = "se", new.coef = new.coef)
    }
    else {
      if (!seonly) 
        est <- predConfPois_homog(window, model, level, estimatename, nCores = nCores,
                            new.coef = new.coef)
      if (se) 
        sem <- predConfPois_homog(window, model, level, "se", nCores = nCores,
                            new.coef = new.coef)
    }
    if (!se) 
      return(est)
  ...
  if (trivial) {
    ...
    if (needSE) {
      if (inherits(glmfit, "gam")) {
        ...
        vv <- quadform(mm, vc)
        SE <- lambda * sqrt(vv) / nCores #sqrt(nCores)
      }
      if (se) 
        zse <- SE
